...
def load_images(root_dir, img_size=(64, 64)):
    images, labels = [], []
    class_names = sorted(os.listdir(root_dir))  # List of class names
    ...
    for label, folder in enumerate(class_names):
        ...
        for filename in os.listdir(folder_path):
            ...
            labels.append(label)  # Assign class number (0, 1, 2...)
...
# 1764
# === Step 5: Test for different PCA dimensions (if enabled) ===
if USE_PCA:
    if USE_HOG:
    ...
    plt.title("Accuracy vs PCA Components and " + technics + " with Logistic Regression")
    plt.grid(True)
    plt.show()

else:
    # If PCA is not enabled, use raw data
    print("🔹 Training Logistic Regression model without PCA...")
    model = LogisticRegression(max_iter=1000)
